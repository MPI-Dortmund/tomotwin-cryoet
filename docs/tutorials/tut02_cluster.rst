...
.. image:: ../img/tutorial_2/find_clusters_targets_overview.png.png
   :align: right
   :width: 350
...
* **Tools**: Here you will find some helpful tools. First you need to select a cluster from the dropdown box. :guilabel:`Show target` will help you evaluate if a cluster might be a good target. :guilabel:`Recompute UMAP` allows you to refine a selected cluster. Once you have found a good cluster, you can add it to the candidate list with "Add candidate".
* **Candidates**: Each row in this table represents a candidate target. You can change the label if you want to. Left clicking on the table allows to :guilabel:`Show` or the :guilabel:`Delete` the candidate.
