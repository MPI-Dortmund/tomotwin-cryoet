...
* **Candidates**: Each row in this table represents a candidate target. You can change the label if you want to. Left clicking on the table allows to :guilabel:`Show` or the :guilabel:`Delete` the candidate. You can save the candidate targets to disk with :guilabel:`Save candidates`.
