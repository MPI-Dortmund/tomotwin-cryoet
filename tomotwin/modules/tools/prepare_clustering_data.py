...
parser.add_argument('--subset', type=bool, default=False,
                    help='If set to true, only 500,000 subvolumes will be transformed and used for the segmentation mask')
...
def _main_():
    out_pth = args.output
    umap_embeddings = calculate_umap(embeddings=args.input, fit_sample_size=args.fit_sample_size, transform_chunk_size=args.chunk_size, subset=args.subset)
    os.makedirs(out_pth, exist_ok=True)
    print("Write umap embeddings to disk")
    umap_embeddings[0].to_csv(
        os.path.join(out_pth, os.path.splitext(os.path.basename(args.input))[0] + "_umap.csv"))
    segmentation_layer = create_segmentation_map(embeddings=umap_embeddings[1])
